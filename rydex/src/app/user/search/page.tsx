...
function page() {
  ...
  const [pickUpLat, setPickUpLat] = useState(Number(params.get("pickuplat")));
  const [pickUpLon, setPickUpLon] = useState(Number(params.get("pickuplon")));
  // const dropLat = Number(params.get("droplat"));
  // const dropLon = Number(params.get("droplon"));
  const [dropLat, setDropLat] = useState(Number(params.get("droplat")));
  const [dropLon, setDropLon] = useState(Number(params.get("droplon")));
  ...
  useEffect(() => {
    console.log("Searching with:", pickUpLat, pickUpLon);
    ...
    <div className="min-h-screen bg-zinc-100 text-zinc-900 overflow-x-hidden">
      ...
      <div className="relative w-full h-[52vh] z-0">
        <SearchMap
          ...
          drop={drop}
          onDistance={setKm}
          onChange={(p, d, pLat, pLon, dLat, dLon) => {
            setPickUp(p);
            setDrop(d);

            if (pLat !== undefined) setPickUpLat(pLat);
            if (pLon !== undefined) setPickUpLon(pLon);
            if (dLat !== undefined) setDropLat(dLat);
            if (dLon !== undefined) setDropLon(dLon);
          }}
        />
      </div>
        ...
        <div className="px-5 lg:px-8 max-w-6xl mx-auto">
          ...
          >
            <div className="flex gap-3 px-4 py-3 border-b border-zinc-100">
              ...
              <MapPin size={14} className="text-zinc-400 flex-shrink-0 m" />
            </div>
            ...
            <div className="flex gap-3 px-4 py-3 border-b border-zinc-100">
              ...
              <Navigation size={14} className="text-zinc-400 flex-shrink-0 m" />
            </div>
          </motion.div>
